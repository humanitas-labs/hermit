...
| S5 preset conflict           | `preset: private` with a third-party provider also configured; the run selects the third-party model                          | refused before any request, error naming the destination and preset; the local model stays usable (a startup error was the original spec and was dropped as hostile to users who keep a cloud key alongside a local model) |
